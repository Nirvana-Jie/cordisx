...
describe('DomOutletController', () => {
  it('rolls back Playground styles and observers before publishing controls when outlet declaration fails', () => {
    ...
    const routes = {
      outlets: {
        declare: vi.fn(() => {
          throw new Error('outlet collision')
        }),
      },
    }
  ...
  it('re-inserts the same CordisX layer after a same-context React anchor replacement', async () => {
    const dom = new JSDOM(
      '<body><main id="anchor" style="position:relative"><div id="native">native</div></main></body>',
    )
  ...
  it('honors an explicit body portal and keeps one geometry observer for the same anchor', () => {
    ...
    const controller = new DomOutletController(
      dom.window.document,
      'custom.panel',
      'portal',
      () => ({ anchor, contextKey: 'panel:one' }),
    )
    controller.show()
...
describe('ReasoningIntensityProjection', () => {
  it('keeps native styles for overlay and restores a hide-native lease', () => {
    const dom = new JSDOM(
      '<body><input id="range" type="range" min="0" max="4" value="2" style="opacity:.7;accent-color:blue"></body>',
    )
    ...
    projection.update(
      ...
      {
        variant: 'imperium',
        title: localized('Intensity'),
        stages: [
          { label: localized('Low'), material: 'plastic' },
          { label: localized('High'), material: 'gold' },
        ],
      },
      'Intensity',
      ['Low', 'High'],
  ...
  it('adapts the current Radix power slider without replacing its native value authority', () => {
    const dom = new JSDOM(
      `<body><div role="menu"><div data-model-picker-power-slider style="height:40px"><span data-orientation="horizontal"><span role="slider" aria-valuemin="0" aria-valuemax="4" aria-valuenow="1" aria-label="Intensity"></span></span></div></div></body>`,
    )
    ...
    vi.spyOn(native, 'getClientRects').mockReturnValue(
      { 0: visibleRect, length: 1, item: () => visibleRect } as DOMRectList,
    )
    let rightKeys = 0
    native.addEventListener('keydown', event => {
      if (event.key === 'ArrowRight') rightKeys += 1
    })
    ...
    expect(dom.window.document.querySelector<HTMLElement>('[data-model-picker-power-slider]')?.style.height).toBe(
      '32px',
    )
    ...
    projection.update(
      range,
      {
        ...
        stages: [
          { label: text('Plastic'), material: 'plastic' },
          { label: text('Bronze'), material: 'bronze' },
          { label: text('Steel'), material: 'steel' },
          { label: text('Silver'), material: 'silver' },
          { label: text('Gold'), material: 'gold' },
        ],
      },
      'Intensity',
      ['Plastic', 'Bronze', 'Steel', 'Silver', 'Gold'],
    )
    range.value = '4'
  ...
  it('adapts the current native reasoning menu into a reversible range', () => {
    ...
    for (const item of items) {
      ...
      vi.spyOn(item, 'getClientRects').mockReturnValue(
        { 0: visibleRect, length: 1, item: () => visibleRect } as DOMRectList,
      )
    }
    ...
    expect(dom.window.document.querySelector<HTMLElement>('.cordisx-reasoning-intensity')?.dataset.material).toBe(
      'gold',
    )
  ...
  it('keeps the native range interactive, animates its projection, and restores styles on cleanup', () => {
    const dom = new JSDOM(
      '<body><input id="range" type="range" min="0" max="4" value="0" style="opacity:.8;accent-color:red"></body>',
    )
    const range = dom.window.document.getElementById('range') as HTMLInputElement
    vi.spyOn(range, 'getBoundingClientRect').mockReturnValue({
      x: 20,
...
describe('SessionBackdropProjection', () => {
  it('follows the native range, retains the last stage after the menu closes, and removes cleanly', () => {
    const dom = new JSDOM(
      '<body><div id="root"><main id="main-surface" style="isolation:auto"><div data-app-shell-main-content-layout="thread-edge-scroll"></div><div id="native-content"></div><input id="range" type="range" min="0" max="4" value="0"></main></div></body>',
    )
    const range = dom.window.document.getElementById('range') as HTMLInputElement
    ...
    const presentation = {
      ...
      driver: 'reasoning-intensity' as const,
      motion: 'ascension' as const,
      stages: [
  ...
  it('mounts only the backdrop layers requested by the structured presentation', () => {
    const dom = new JSDOM(
      '<body><div id="root"><main><div data-app-shell-main-content-layout="thread-edge-scroll"></div></main></div></body>',
    )
    const projection = new SessionBackdropProjection(dom.window.document)
    const text = (key: string) => ({ key, fallback: key })
    const stages = [
      {
        ...
        ambience: 'dormant' as const,
        portrait: { mediaType: 'image/png' as const, data: 'cGxhc3RpYw==', alt: text('plastic') },
      },
      {
        material: 'gold' as const,
        ambience: 'imperial' as const,
        portrait: { mediaType: 'image/png' as const, data: 'Z29sZA==', alt: text('gold') },
      },
    ]
    ...
    projection.update('session-a', undefined, {
      ...
      layers: { portrait: false, effects: true },
      stages,
